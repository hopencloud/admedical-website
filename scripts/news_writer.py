...
[우선순위 — 1·2군이 하나라도 있으면 반드시 거기서 고르세요]
...
2군
  · 병원 마케팅 채널 변화 (플랫폼 수수료, SNS, 검색, 앱)
  · 진료과별 수요·개원 시장 변화

3군 (1·2군이 전혀 없을 때만)
  · 보건복지부 정책 중 병의원 운영·수익에 직접 닿는 것

[3군도 안 되는 것 — 고르면 안 됩니다]
전공의 수급, 학회 연구 결과, 병원 인력 구조, 응급의료 체계, 약사법,
질병 치료법·신의료기술 같은 임상·정책 일반 뉴스.
독자는 **병의원 마케팅 담당자**입니다. 진료가 아니라 광고를 만드는 사람입니다.
"이 소식이 마케팅에 영향을 줄 수도 있다"는 식으로 억지로 엮지 마세요.
마땅한 게 없으면 selected 를 false 로 하세요. 억지로 쓴 글은 아무도 안 읽습니다.

[제외 — 하나라도 해당하면 절대 고르지 마세요]
- **업체·병원·플랫폼의 홍보성 소식.** 리뉴얼·오픈·출시·제휴·수상·주년·기부·실적 등.
  회사 이름이나 서비스 이름이 주어인 기사는 대부분 보도자료입니다.
  "그 소식이 마케팅에 영향을 준다"는 식으로 억지로 엮지 마세요.
  (실제로 한 업체의 플랫폼 리뉴얼 보도자료가 기사로 나간 적이 있습니다)
- 인사·승진·학회 개최·행사 안내
- 제약사 실적·신약 허가 (병의원 마케팅과 무관)
- 이미 다룬 주제: {published}

판단 기준: **그 회사가 사라져도 성립하는 기사인가?** 아니면 고르지 마세요.
제도·규제·정책·판례처럼 업계 전체에 적용되는 사안만 고릅니다.
...
[검색용 제목(seo_title) — 클릭률이 여기서 갈립니다]
구글 검색결과에 실제로 뜨는 제목입니다. 지금 이 사이트는 평균 7.6등(첫 페이지)에
노출되는데도 클릭률이 0.5%밖에 안 됩니다. 제목에 사람들이 검색하는 말이 없어서입니다.

- **맨 앞 15자 안에 독자가 검색창에 칠 말을 넣으세요.**
  좋음: "의료광고 심의", "병원 마케팅", "비급여 광고", "의료법 위반", "불법 의료광고"
  나쁨: "다학제팀의료", "제도화 필요성", "정책 변화 예상", "선례", "논의 중"
- 그 뒤에 **이 기사에만 있는 구체적인 내용**을 붙이세요.
- 55자 이내. 넘으면 구글이 잘라냅니다.
- "~필요성", "~예상", "~논의" 처럼 흐지부지한 끝맺음 금지.
  무엇이 어떻게 바뀌는지 단정적으로 쓰세요 (사실 범위 안에서).

  예) 나쁨: 입원환자 다학제팀의료 제도화 필요성
      좋음: 병원 홍보에 '다학제 진료' 쓸 때 심의 주의점
...
{{
  ...
  "seo_title": "검색결과에 뜨는 제목. 아래 [검색용 제목] 규칙을 반드시 지킬 것",
