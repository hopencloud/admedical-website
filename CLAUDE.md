...
`collector.py` 가 내려받는 폴더와 `batch_vision_ocr.py` 가 읽는 폴더는 **반드시 같아야 한다.**
셋 다 `기존데이터/수집` (환경변수 `ADMEDICAL_SAVE_DIR` 로 오버라이드, 클라우드는 /tmp).

예전에는 collector 만 `~/Desktop/admedical_ads` 를 기본값으로 써서
새로 받은 시안이 OCR 대상에서 통째로 빠지는 구조였다. 신규 시안이 없던 기간이라
증상이 '바탕화면에 빈 폴더가 계속 생긴다'로만 보였을 뿐이다.
